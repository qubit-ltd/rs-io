...
fn fuzz_buffered_input(data: &[u8]) {
    ...
    let mut input =
        BufferedInput::with_capacity(Cursor::new(source.to_vec()), capacity);
    let mut position = 0;

    for operation in source.chunks(3) {
        let opcode = operation.first().copied().unwrap_or_default() % 3;
        let argument =
            usize::from(operation.get(1).copied().unwrap_or_default());
        match opcode {
            ...
            1 => {
                ...
                let filled = input
                    .fill_until(count)
                    .expect("cursor reads should not fail");
                assert_eq!(
                    source.len().saturating_sub(position) >= count,
                    filled
                );
            }
            _ => {
        ...
        assert_eq!(
            input.unread(),
            &source[position..position + input.unread_len()]
        );
    }
}
...
fn fuzz_buffered_output(data: &[u8]) {
    let capacity =
        usize::from(data.first().copied().unwrap_or_default() % 32) + 1;
    let mut output =
        BufferedOutput::with_capacity(Cursor::new(Vec::new()), capacity);
    let mut expected = Vec::new();

    for operation in data.get(1..).unwrap_or_default().chunks(3) {
        let opcode = operation.first().copied().unwrap_or_default() % 3;
        let count =
            usize::from(operation.get(1).copied().unwrap_or_default() % 33);
