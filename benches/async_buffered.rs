    ...
    I: AsyncInput<Item = u8> + Unpin,
{
    ...
    loop {
        ...
        match Pin::new(&mut *input).poll_read(&mut context, &mut output[position..end]) {
            ...
            Poll::Ready(Err(error)) => panic!("async input benchmark read failed: {error}"),
    ...
    O: AsyncOutput<Item = u8> + Unpin,
{
    ...
    while position < input.len() {
        ...
        match Pin::new(&mut *output).poll_write(&mut context, &input[position..end]) {
            Poll::Ready(Ok(count)) => position += count,
            Poll::Ready(Err(error)) => panic!("async output benchmark write failed: {error}"),
            Poll::Pending => pending_polls += 1,
        }
    }
    loop {
        ...
        match Pin::new(&mut *output).poll_flush(&mut context) {
            Poll::Ready(Ok(())) => break,
            Poll::Ready(Err(error)) => panic!("async output benchmark flush failed: {error}"),
            Poll::Pending => pending_polls += 1,
        }
    }
    PollStats {
