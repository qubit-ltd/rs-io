...
fn copy_unchecked_with_runtime_indexes(input: &[u8], output: &mut [u8], offsets: &[usize], width: usize) {
...
fn copy_safe_with_runtime_indexes(input: &[u8], output: &mut [u8], offsets: &[usize], width: usize) {
    for &offset in offsets {
        let offset = black_box(offset);
        output[offset..offset + width].copy_from_slice(&input[offset..offset + width]);
    }
...
fn copy_from_safe_slices(input: &[u8], output: &mut [u8], width: usize) {
    for output_index in (0..output.len()).step_by(width) {
        output[output_index..output_index + width].copy_from_slice(&input[output_index..output_index + width]);
    }
}
...
fn benchmark_visible_invariants(criterion: &mut Criterion) {
    ...
    for width in COPY_WIDTHS {
        group.bench_with_input(BenchmarkId::new("unchecked_slice", width), &width, |bencher, &width| {
        ...
        group.bench_with_input(BenchmarkId::new("safe_slices", width), &width, |bencher, &width| {
            bencher.iter_batched(
                || vec![0_u8; DATA_LEN],
                |mut output| {
