...
fn test_leb128_codec_exposes_min_buffer_len() {
    assert_eq!(2, Leb128Codec::<u8, NonStrict>::MIN_BUFFER_LEN);
    assert_eq!(3, Leb128Codec::<u16, NonStrict>::MIN_BUFFER_LEN);
    assert_eq!(5, Leb128Codec::<u32, NonStrict>::MIN_BUFFER_LEN);
    assert_eq!(10, Leb128Codec::<u64, NonStrict>::MIN_BUFFER_LEN);
    assert_eq!(19, Leb128Codec::<u128, NonStrict>::MIN_BUFFER_LEN);
    assert_eq!(
        (usize::BITS as usize).div_ceil(7),
        Leb128Codec::<usize, NonStrict>::MIN_BUFFER_LEN
    );
    assert_eq!(2, Leb128Codec::<i8, Strict>::MIN_BUFFER_LEN);
    assert_eq!(3, Leb128Codec::<i16, Strict>::MIN_BUFFER_LEN);
}

#[test]
fn test_leb128_codec_reads_and_writes_unsigned_values_unchecked() {
    let mut output = [0u8; Leb128Codec::<u16, NonStrict>::MIN_BUFFER_LEN + 2];
    let len = unsafe { Leb128Codec::<u16, NonStrict>::write_unchecked(&mut output, 1, 300) };

    assert_eq!(2, len);
    assert_eq!([0x00, 0xac, 0x02, 0x00, 0x00], output);

    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::read_unchecked(&output, 1) }
        .expect("valid u16 should decode");
    assert_eq!((300, 2), decoded);

    let mut output = [0u8; Leb128Codec::<u16, NonStrict>::MIN_BUFFER_LEN];
    let len = unsafe { Leb128Codec::<u16, NonStrict>::write_unchecked(&mut output, 0, u16::MAX) };
    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::read_unchecked(&output, 0) }
...
fn test_leb128_codec_reads_and_writes_signed_values_unchecked() {
    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MIN_BUFFER_LEN + 2];
    let len = unsafe { Leb128Codec::<i16, NonStrict>::write_unchecked(&mut output, 1, -300) };

    assert_eq!(2, len);
    assert_eq!([0x00, 0xd4, 0x7d, 0x00, 0x00], output);

    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::read_unchecked(&output, 1) }
        .expect("valid i16 should decode");
    assert_eq!((-300, 2), decoded);

    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MIN_BUFFER_LEN];
    ...
    let mut output = [0u8; Leb128Codec::<i128, NonStrict>::MIN_BUFFER_LEN];
    let len = unsafe { Leb128Codec::<i128, NonStrict>::write_unchecked(&mut output, 0, i128::MIN) };
    let decoded = unsafe { Leb128Codec::<i128, NonStrict>::read_unchecked(&output, 0) }
...
fn test_leb128_codec_roundtrips_all_strict_and_non_strict_instantiations() {
    macro_rules! roundtrip_unsigned {
        ($ty:ty, $value:expr) => {{
            ...
            let mut output = [0u8; Leb128Codec::<$ty, NonStrict>::MIN_BUFFER_LEN];
            let len =
                unsafe { Leb128Codec::<$ty, NonStrict>::write_unchecked(&mut output, 0, value) };
            let decoded = unsafe { Leb128Codec::<$ty, NonStrict>::read_unchecked(&output, 0) }
                .expect("non-strict unsigned value should decode");
            assert_eq!((value, len), decoded);

            let mut output = [0u8; Leb128Codec::<$ty, Strict>::MIN_BUFFER_LEN];
...
fn test_leb128_codec_rejects_all_instantiated_error_paths() {
    macro_rules! reject_unsigned {
        ($ty:ty) => {{
            let unterminated = [0x80u8; Leb128Codec::<$ty, NonStrict>::MIN_BUFFER_LEN];
            ...
            let max_bytes = Leb128Codec::<$ty, NonStrict>::MIN_BUFFER_LEN;
            let bits = <$ty>::BITS as usize;
            let used_bits = bits - (max_bytes - 1) * 7;
            let mut malformed = [0x80u8; Leb128Codec::<$ty, NonStrict>::MIN_BUFFER_LEN];
            ...
            let mut noncanonical = [0u8; Leb128Codec::<$ty, Strict>::MIN_BUFFER_LEN];
            noncanonical[0] = 0x80;
