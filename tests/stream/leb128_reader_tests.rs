...
fn test_leb128_reader_read_utf8_string_reads_length_prefixed_payload() {
    ...
    let mut reader = qubit_io::Leb128Reader::<_, qubit_io::NonStrict>::new(std::io::Cursor::new(bytes));
...
fn test_leb128_reader_read_and_seek_delegate_to_inner_reader() {
    let mut reader = qubit_io::Leb128Reader::<_, qubit_io::NonStrict>::new(std::io::Cursor::new(vec![1, 2, 3, 4]));
