...
impl ScriptedOutput {
    fn new(write_steps: impl IntoIterator<Item = WriteStep>, flush_steps: impl IntoIterator<Item = FlushStep>) -> Self {
        Self {
            values: Vec::new(),
...
#[test]
fn test_async_buffered_input_preserves_generic_items_across_pending_refills() -> io::Result<()> {
...
fn test_async_buffered_output_retains_items_until_async_flush() -> io::Result<()> {
    let inner = ScriptedOutput::new([WriteStep::Pending, WriteStep::Accept(2), WriteStep::Accept(1)], []);
...
fn test_async_buffered_io_supports_clone_only_items() -> io::Result<()> {
    let mut input = AsyncBufferedInput::with_capacity(StringAsyncInput::new(["alpha", "beta", "gamma"]), 2);
    let mut read = vec![String::default(); 3];

    assert_eq!(3, complete(input.read_fully_async(&mut read))?);
    assert_eq!(read, ["alpha", "beta", "gamma"]);

    let mut output = AsyncBufferedOutput::with_capacity(StringAsyncOutput::default(), 2);
...
#[test]
fn test_async_buffered_input_exposes_owned_parts_and_accessors() -> io::Result<()> {
    ...
    let input = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Eof]), 0);
...
fn test_async_buffered_input_handles_empty_direct_pending_eof_and_error_reads() {
    ...
    let mut empty = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![1])]), 2);
    let mut output = [];
    assert_eq!(
        0,
        Pin::new(&mut empty)
    ...
    let mut direct = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![3, 4, 5])]), 2);
    ...
    let mut pending =
        AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Pending, ReadStep::Data(vec![6])]), 2);
    let mut output = [0_u16; 1];
    assert!(Pin::new(&mut pending).poll_read(&mut cx, &mut output).is_pending());
    assert_eq!(
        1,
        Pin::new(&mut pending)
            .poll_read(&mut cx, &mut output)
    ...
    let mut eof = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Eof]), 2);
    assert_eq!(
        0,
        Pin::new(&mut eof)
            .poll_read(&mut cx, &mut output)
    ...
    let mut failing =
        AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Error(ErrorKind::PermissionDenied)]), 2);
...
#[test]
fn test_async_buffered_output_exposes_pending_parts_and_direct_writes() -> io::Result<()> {
    ...
    let mut direct = AsyncBufferedOutput::with_capacity(ScriptedOutput::new([], []), 2);
...
#[test]
fn test_async_buffered_output_writes_exact_capacity_directly_when_empty() -> io::Result<()> {
    let mut output = AsyncBufferedOutput::with_capacity(ScriptedOutput::new([], []), 2);
...
#[test]
fn test_async_buffered_output_buffers_write_that_exactly_fills_remaining_capacity() -> io::Result<()> {
    let mut output = AsyncBufferedOutput::with_capacity(ScriptedOutput::new([], []), 2);
...
#[test]
fn test_async_buffered_output_handles_pending_and_partial_drains() -> io::Result<()> {
    let inner = ScriptedOutput::new(
        [
            WriteStep::Pending,
            WriteStep::Accept(1),
    ...
    assert_eq!(
        1,
        Future::poll(Pin::new(&mut future), &mut cx).expect_ready("draining write should complete")?
    );
    drop(future);
    ...
    assert!(Future::poll(Pin::new(&mut future), &mut cx).is_pending());
    Future::poll(Pin::new(&mut future), &mut cx).expect_ready("inner flush should complete")?;
    drop(future);
    assert_eq!(&[1, 2, 3], output.inner().values.as_slice());
    assert_eq!(0, output.pending_len());
    Ok(())
...
#[test]
fn test_async_buffered_output_reports_drain_and_flush_failures() -> io::Result<()> {
    let mut zero = AsyncBufferedOutput::with_capacity(ScriptedOutput::new([WriteStep::Zero], []), 2);
    assert_eq!(1, complete(zero.write_async(&[1]))?);
    let error = complete(zero.write_async(&[2, 3])).expect_err("zero-length drain should fail");
    assert_eq!(ErrorKind::WriteZero, error.kind());
    assert_eq!(&[1], zero.pending());

    let mut failing_write =
        AsyncBufferedOutput::with_capacity(ScriptedOutput::new([WriteStep::Error(ErrorKind::BrokenPipe)], []), 2);
    assert_eq!(1, complete(failing_write.write_async(&[1]))?);
    let error = complete(failing_write.write_async(&[2, 3])).expect_err("drain error should be preserved");
    ...
    let error = complete(failing_drain_flush.flush_async()).expect_err("flush should preserve its drain error");
    assert_eq!(ErrorKind::ConnectionReset, error.kind());

    let mut failing_inner_flush =
        AsyncBufferedOutput::with_capacity(ScriptedOutput::new([], [FlushStep::Error(ErrorKind::Other)]), 1);
    let error = complete(failing_inner_flush.flush_async()).expect_err("inner flush error should be preserved");
...
fn test_async_buffered_output_rejects_forbidden_flush_and_close_error_kinds() {
    for kind in [ErrorKind::WouldBlock, ErrorKind::Interrupted] {
        let mut flushing = AsyncBufferedOutput::new(ScriptedOutput::new([], [FlushStep::Error(kind)]));
        let mut closing = AsyncBufferedOutput::new(ScriptedOutput::new([], []).with_close_error(kind));
        let mut cx = Context::from_waker(Waker::noop());
        ...
        let close_error = AsyncClose::poll_close(Pin::new(&mut closing), &mut cx)
...
fn test_async_buffered_output_drains_before_closing_inner() -> io::Result<()> {
    ...
    AsyncClose::poll_close(Pin::new(&mut output), &mut cx).expect_ready("close should complete")?;
...
#[test]
fn test_async_buffered_output_close_preserves_pending_data_while_drain_is_pending() -> io::Result<()> {
    let inner = ScriptedOutput::new([WriteStep::Pending, WriteStep::Accept(2)], []);
    let mut output = AsyncBufferedOutput::with_capacity(inner, 3);
    assert_eq!(2, complete(output.write_async(&[1, 2]))?);
    let mut cx = Context::from_waker(Waker::noop());
    ...
    AsyncClose::poll_close(Pin::new(&mut output), &mut cx).expect_ready("close should finish after draining")?;
    assert_eq!(&[1, 2], output.inner().values.as_slice());
    assert!(output.inner().closed);
    assert_eq!(0, output.pending_len());
    Ok(())
...
fn test_async_buffered_output_close_preserves_drain_error() -> io::Result<()> {
    let inner = ScriptedOutput::new([WriteStep::Error(ErrorKind::BrokenPipe)], []);
    let mut output = AsyncBufferedOutput::with_capacity(inner, 3);
    assert_eq!(2, complete(output.write_async(&[1, 2]))?);
    let mut cx = Context::from_waker(Waker::noop());
...
fn test_async_buffered_input_exposes_window_refill_and_consume_operations() -> io::Result<()> {
    ...
    input.try_reserve_capacity(6).expect("buffer capacity should grow");
    assert!(input.capacity() >= 6);
    Ok(())
}

#[test]
fn test_async_buffered_input_yields_after_bounded_ready_refills() -> io::Result<()> {
    let inner = ScriptedInput::new((0_u16..65).map(|item| ReadStep::Data(vec![item])));
    let mut input = AsyncBufferedInput::with_capacity(inner, 65);
    let mut cx = Context::from_waker(Waker::noop());

    assert!(Pin::new(&mut input).poll_fill_until(&mut cx, 65).is_pending());
...
fn test_async_buffered_input_ensure_available_discards_incomplete_eof_window() {
    let mut input = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![1]), ReadStep::Eof]), 2);
    let mut cx = Context::from_waker(Waker::noop());
...
fn test_async_buffered_input_covers_refill_error_and_pending_paths() {
    ...
    let mut full = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![1, 2])]), 2);
    assert!(
        Pin::new(&mut full)
    ...
    let mut failing =
        AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Error(ErrorKind::BrokenPipe)]), 2);
    let error = Pin::new(&mut failing)
        .poll_fill_more(&mut cx)
    ...
    let mut pending = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Pending]), 2);
    assert!(Pin::new(&mut pending).poll_fill_more(&mut cx).is_pending());

    let mut invalid_count = AsyncBufferedInput::with_capacity(ScriptedInput::new([]), 2);
    ...
    let mut failed_until =
        AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Error(ErrorKind::ConnectionReset)]), 2);
    ...
    let mut pending_until = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Pending]), 2);
    assert!(Pin::new(&mut pending_until).poll_fill_until(&mut cx, 1).is_pending());

    let mut ready_ensure = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![9])]), 2);
    Pin::new(&mut ready_ensure)
        .poll_ensure_available(&mut cx, 1)
        .expect_ready("available item should satisfy ensure")
        .expect("ensure should succeed");

    let mut failed_ensure =
        AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Error(ErrorKind::Other)]), 2);
    ...
    let mut pending_ensure = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Pending]), 2);
    assert!(
    ...
    let mut zero_read = AsyncBufferedInput::with_capacity(ScriptedInput::new([ReadStep::Data(vec![1])]), 2);
    let mut output = [];
    assert_eq!(
        0,
        unsafe { Pin::new(&mut zero_read).poll_read_unchecked(&mut cx, &mut output, 0, 0,) }
...
fn test_async_buffered_output_exposes_spare_window_and_async_capacity_check() -> io::Result<()> {
    ...
    output.try_reserve_capacity(6).expect("buffer capacity should grow");
    assert!(output.capacity() >= 6);
    Ok(())
}

#[test]
fn test_async_buffered_output_yields_after_bounded_ready_drains() -> io::Result<()> {
    let inner = ScriptedOutput::new(std::iter::repeat_with(|| WriteStep::Accept(1)).take(65), []);
...
#[test]
fn test_async_buffered_input_async_fill_helpers_delegate_to_poll_operations() -> io::Result<()> {
    let inner = ScriptedInput::new([ReadStep::Data(vec![10]), ReadStep::Data(vec![20])]);
...
#[test]
fn test_async_buffered_output_async_capacity_helper_drains_pending_items() -> io::Result<()> {
    let mut output = AsyncBufferedOutput::with_capacity(ScriptedOutput::new([], []), 3);
    let (spare, index, available) = output.spare_raw_parts_mut();
    assert_eq!(0, index);
    assert_eq!(3, available);
