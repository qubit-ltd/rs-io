...
fn test_temp_file_with_name_uses_system_temp_directory() {
    let file = TempFile::with_name(Some("qubit-io-test-"), Some(".tmp"))
...
fn test_temp_guards_support_debug_formatting() {
    let file = TempFile::with_name(Some("qubit-io-debug-"), Some(".tmp"))
        .expect("temp file should be created");
    let dir =
        TempDir::with_prefix(Some("qubit-io-debug-")).expect("temp directory should be created");
...
fn test_temp_dir_with_prefix_creates_existing_directory() {
    let dir =
        TempDir::with_prefix(Some("qubit-io-dir-")).expect("temp directory should be created");
    let name = dir
        .path()
        .file_name()
