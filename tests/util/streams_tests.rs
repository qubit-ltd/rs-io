...
use qubit_io::{DEFAULT_BUFFER_CAPACITY, Input, Output, Streams};
...
struct StringItemInput {
    values: VecDeque<String>,
}

impl StringItemInput {
...
fn test_copy_input_to_output_copies_all_remaining_items() {
    ...
    let copied =
        Streams::copy_input_to_output(&mut input, &mut output).expect("item copy should reach EOF");
...
#[test]
fn test_copy_item_streams_support_clone_only_items() {
    let mut input = StringItemInput::new(["alpha", "beta", "gamma"]);
    let mut output = StringItemOutput::default();
    ...
    let mut input = StringItemInput::new(["delta", "epsilon"]);
    let mut output = StringItemOutput::default();
    assert_eq!(
        1,
        Streams::copy_input_to_output_at_most(&mut input, &mut output, 1)
            .expect("bounded string item copy should succeed")
    );
    assert_eq!(output.values, ["delta"]);

    let mut input = StringItemInput::new(["zeta", "eta"]);
    let mut output = StringItemOutput::default();
    assert_eq!(
        2,
        Streams::copy_input_to_output_end_limited(&mut input, &mut output, 2)
            .expect("length-limited string item copy should succeed")
    );
    assert_eq!(output.values, ["zeta", "eta"]);
}
...
fn test_copy_at_most_with_buffer_size_returns_allocation_error() {
    ...
    let error =
        Streams::copy_at_most_with_buffer_size(&mut input, &mut output, u64::MAX, usize::MAX)
