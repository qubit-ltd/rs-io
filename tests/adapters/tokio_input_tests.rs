...
impl AsyncRead for ErrorReader {
    fn poll_read(self: Pin<&mut Self>, _cx: &mut Context<'_>, _buffer: &mut ReadBuf<'_>) -> Poll<std::io::Result<()>> {
...
fn test_tokio_input_rejects_forbidden_error_kinds() {
    for kind in [ErrorKind::WouldBlock, ErrorKind::Interrupted] {
        ...
        let result = unsafe { AsyncInput::poll_read_unchecked(Pin::new(&mut input), &mut cx, &mut buffer, 0, 1) };
